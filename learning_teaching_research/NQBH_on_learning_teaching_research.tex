...
\usepackage{amsmath,amssymb,amsthm,enumitem,fancyvrb,float,glossaries,graphicx,mathtools,multicol,musicography,soul,subfig,tikz,tipa}
...
\begin{enumerate}
	\item {\sf Confession on linguistics -- Thú tội về mặt ngôn ngữ}: The 2 authors are 2 (?) typical linguistic retarded, having no linguistic abilities.
	
	-- 2 tác giả là 2 kẻ dốt ngôn ngữ điển hình, không có 1 chút năng khiếu nào về ngôn ngữ.
...
\end{enumerate}
Những lúc buồn chán \& thất vọng, bạn thường có những suy nghĩ như thế. Thay vì tự ngâm nga bài hát \musEighth[\href{https://www.youtube.com/watch?v=bqzDuRz_P7g}{\it Don't Know What To Do}]\musEighth\ của nhóm nhạc Hàn Quốc {\sc BlackPink} nổi tiếng.
...
\subsection{A ``warm'' \&{\tt/}but cold Christmas}
...
\section{On Writing: Literary Writing for A Literary Retard -- Bàn Về Việc Viết: Học Viết Văn Cho Kẻ Dốt Đặc Văn Chương}
...
\subsubsection{Dictionary -- Từ điển}
We have used:
\begin{itemize}
...
\subsubsection{Translation operator -- Toán tử dịch thuật}
\textbf{\textsf{\small Vocabulary -- Từ vựng.}}
\begin{itemize}\small
...
\subsubsection{Writing standards -- Các tiêu chuẩn viết lách}
...
\subsubsection{Some rules \& principles in writing}
Some of own rules \& principles in writing our crafts are the following:

-- Vài quy tắc \& nguyên tắc trong việc viết các bản thảo của chúng tôi:
\begin{enumerate}
	...
	-- Cung cấp các từ vựng cần thiết cho 1 khái niệm mới hoặc 1 khái niệm cũ nhưng quan trọng, \& các gia đình từ tương ứng.
	\item Always cite clearly references. Use quotations as accurate as possible.
	
	-- Luôn trích dẫn rõ ràng các nguồn tài nguyên (có thể là nguồn tài liệu, nguồn hình ảnh, nguồn video, etc.). Sử dụng các dấu ngoặc kép{\tt/}nháy đôi để chỉ rõ các phần được trích dẫn (nhưng cũng cần phân biệt với việc sử dụng dấu nháy đôi để châm biếm 1 từ với nghĩa ngược lại).
	\item Learn from mistakes, failed drafts \& crafts (`failed' here means failure or at least in the standards of its author(s)).
	
	-- Học hỏi từ những sai lầm, những bản nháp viết sai, những bản thảo thất bại, hoặc ít nhất được coi là thất bại so với ý muốn của tác giả của các lỗi sai, các thất bại đó.
	\item Do not type the whole books: only extract good{\tt/}general{\tt/}emotional{\tt/}deep sentences{\tt/}paragraphs.
	
	-- Đừng gõ toàn bộ cuốn sách: chỉ trích xuất những câu{\tt/}đoạn hay{\tt/}tổng quát{\tt/}giàu cảm xúc{\tt/}sâu sắc{\tt/}hay.
\end{enumerate}
...
\begin{question}[Split vs. Merge -- Chia tách vs. Kết hợp]
	...
	-- Khi nào chúng ta nên tách? Khi nào chúng ta nên gộp? Đặc biệt là khi nào chúng ta nên làm cả 2 điều này (tách \& gộp) tuần tự, luân phiên nhau, \& đồng thời? Làm cách nào để làm tất cả những điều này, i.e., tách, gộp, tách--gộp kết hợp, đúng cách \& hài hòa?
\end{question}
...
\subsection{Dumbphone vs. smartphone -- Điện thoại ngu{\tt/}cùi bắp vs. Điện thoại khôn{\tt/}thông minh}
...
\subsection{Various types of flows -- Các loại dòng chảy khác nhau}
...
\subsubsection{Flows in Mathematics realm -- Các dòng chảy trong địa hạt Toán Học}
...
\subsubsection{Flows in Psychology realm  -- Các dòng chảy trong địa hạt Tâm Lý Học}
...
\subsubsection{Flows in Philosophy realm -- Các dòng chảy trong địa hạt Triết Học}

\subsubsection{Flows in Music realm -- Các dòng chảy trong địa hạt Âm Nhạc}
...
\begin{itemize}
	...
	-- Tôi tự nhủ rằng đừng cảm thấy áp lực về thời gian, rằng mỗi khoảnh khắc bạn đầu tư vào việc quan sát, khám phá, học tập \& tận hưởng những gì bạn thích làm, tất cả đều trở thành một phần của việc trở thành con người bạn muốn trở thành.
...
\begin{Rule}[Laminar flow vs. turbulent flow -- Dòng chảy tầng vs. dòng chảy rối{\tt/}hỗn loạn]
	...
	-- Tự phân tích (các) dòng chảy (các) suy nghĩ của bạn: Liệu dòng chảy đó là dòng chảy tầng hay dòng chảy hỗn loạn?
\end{Rule}

%------------------------------------------------------------------------------%

\subsection{Contributions {\it\&} Legacies -- Sự cống hiến {\it\&} Các di sản}
...
\section{On Research: Stage 1 -- Bàn Về Nghiên Cứu: Hồi 1}
...
\subsection{Dirty trick -- Thủ đoạn bẩn thỉu{\tt/}Mưu hèn kế bẩn}
...
\begin{question}[Dirty trick -- Mưu hèn kế bẩn]
	...
	-- Tại sao chúng ta nên nghiên cứu về các mưu hèn kế bẩn \& hiểu cách các mưu kế này thực sự hoạt động?
\end{question}
...
\begin{quote}
	...
	-- 1 vi phạm phổ biến về tính cô đọng là trình bày 1 ý tưởng phức tạp duy nhất, từng bước 1, trong 1 loạt các câu có thể có lợi khi kết hợp thành một câu.
\end{quote}
...
\subsubsection{Thoroughness -- Tính triệt để}
...
\subsection{A Journey to Psychology}
...
\subsection{Introduction to Dark Psychology}
...
\begin{psy-theorem}[$\Psi$-manipulation: coward $\mapsto$ hero]
	...
	-- Thao túng tâm lý là cách duy nhất để biến một kẻ hèn nhát trở thành anh hùng.
\end{psy-theorem}
...
Nhưng giờ thì mọi thứ đã trở nên rõ ràng \& hợp lý hơn rất nhiều. Mình cũng chả trách gì, xem ra cái suất Marie--Curie này chả dễ ngốn tí nào (see the similar situation for this Marie--Curie PhD). Mình cũng đã giải thích sự tình \& hòa giải với anh bạn Đức khó tánh ở đoạn \fbox{3}, vì mình cảm nhận, thông qua vài hành động của ảnh, cả 2 thằng mình đều là những kẻ chính trực ngu ngốc, khờ dại, \& đáng thương hại. Cả 2 thằng đều bị kẻ thao túng tâm lý giở trò{\tt/}đâm thọt sau lưng, tánh ảnh nóng nên quạo, còn tánh mình hiền nên quỵ.
...
\subsubsection{{\it Emotions}: good{\tt/}bad? {\it Emotionless?}}
...
\subsubsection{Survivors -- Những kẻ sống sót}
...
\begin{quote}
	...
	{\sf Hồng [25; psychologist wannabe]}: Ừ nhỉ. Cũng đúng đúng. Chắc tao cần quan sát, nghiên cứu 1 tí Nhân Diện Học \& Diện Tướng Học\footnote{``Mỗi bắp thịt trên mặt có thể là tiêu biểu của 1 loại tình. Ta nên để ý nghiên cứu về khoa ``Diện tướng học'', 1 khoa học rất bổ ích cho đời sống hàng ngày của ta. Không có 1 tình cảm nào mà nhà quan sát sành sỏi không nhận được trên nét mặt của kẻ khác.'' -- \cite[Chap. 1: Thuật Quan sát, p. 39]{Can_oss}.}, \& chiêm nghiệm thêm mới được.
...
-- Bắt nạt không bao giờ chấm dứt nếu bạn không làm gì để ngăn chặn nó. Bắt nạt chỉ trở nên tốt hơn, \& tốt hơn cho những kẻ bắt nạt, tức là, nó trở nên tệ hơn \& tệ hơn cho những người bị bắt nạt. Nếu tệ hơn là tốt hơn, thì bạn chắc chắn thích BDSM (bao gồm trói buộc, kỷ luật, thống trị, \& phục tùng, khổ dâm.

\subsubsection{De-operator -- Toán tử Triệt Tiêu}
\textbf{\textsf{\small Vocabulary -- Từ vựng.}}
\begin{itemize}\small
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item {\it``Taking care of {\sc ourselves} often becomes a luxury we can't afford, rather than a necessity we can't ignore.''}
...
\section{On Teaching: Stage 1 -- Bàn Về Việc Dạy: Hồi 1}
\label{sect: teaching}
{\bf \textsf{Resources -- Tài nguyên.}}
\begin{enumerate}
...
\begin{Rule}[On watching TED Talk]
	...
	-- Khuyến khích học sinh tìm hiểu các video TED Talk trên YouTube hoặc trang chính thức của TED để học hỏi \& mở rộng kiến thức.
\end{Rule}
...
\subsubsection{Teaching in modern day -- Nghề dạy học trong thời hiện đại}
...
\subsubsection{Design principle -- Nguyên lý thiết kế}
...
\subsubsection{Purposes of a problem -- Các mục đích của 1 bài toán}
...
\subsection{Series: Some Topics in Elementary STEM \& Beyond -- Chuỗi Tài Liệu \& Sách: 1 Số Chủ Đề STEM Sơ Cấp \& Hơn Thế Nữa}
...
\paragraph{Elementary Physics Grade 6 -- Vật Lý Sơ Cấp lớp 6}
\textbf{\textsf{Resources -- Tài nguyên.}}
\begin{enumerate}
...
\paragraph{Elementary Chemistry Grade 6.} Hóa Học Sơ Cấp lớp 6 khá đơn giản, chỉ nêu lên 1 số khái niệm cơ bản, chưa có công thức hóa học, \& các phương trình hóa học. Nên chúng tôi chỉ sử dụng 1 file duy nhất để tóm tắt kiến thức Hóa Học Sơ Cấp lớp 6, thậm chí không cần cheatsheet:
\begin{itemize}
...
\paragraph{Elementary Chemistry Grade 7}

\paragraph{Elementary Chemistry Grade 8}

\paragraph{Elementary Chemistry Grade 9}

\paragraph{Elementary Chemistry Grade 10}
...
\paragraph{Elementary Chemistry Grade 11}
...
\paragraph{Elementary Chemistry Grade 12}
...
\begin{enumerate}\sf\small
	\item {\bf specialization} [n] the process of becoming an expert in a particular area of work, study or business; the fact of spending more time on one area of work, etc. than on others; specialization (in something) a particular area of work, study or business which somebody spends more time on than on other areas.
	\item {\bf generalization} [n] a general statement that is based on only a few facts or examples; the act of making such statements.
\end{enumerate}
...
\subsection{On teaching power of habits -- Bàn về việc dạy sức mạnh của các thói quen}
{\bf \textsf{Resources -- Tài nguyên.}}
\begin{enumerate}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		-- Các văn phòng có kế hoạch mở được cho là làm giảm năng suất \& làm suy giảm trí nhớ.
		
		\item {\it``Peer pressure, in other words, is not only unpleasant, but can actually change your view of a problem.''}
		
		-- Nói cách khác, áp lực từ bạn bè không chỉ gây khó chịu mà còn thực sự có thể thay đổi quan điểm của bạn về một vấn đề.
		
		\item {\it``The ``Bus to Abilene'' anecdote reveals our tendency to follow those who initiate action -- any action.''}
		
		-- Giai thoại “Xe buýt đến Abilene” tiết lộ xu hướng của chúng ta là đi theo những người khởi xướng hành động -- bất kỳ hành động nào.
	\end{itemize}
	Với bản dịch tiếng Việt:
...
\subsection{On book smart vs. street smart -- Bàn về trí thông minh sách vở vs. trí thông minh đường phố}
...
\section{On Learning: Stage 1 -- Bàn Về Việc Học: Hồi 1}
\label{sect: learning}
...
\begin{question}[Learn: What? Why? How? When? Which? Where? Who? With whom?]
	What should we learn?  How should we learn? Why should we learn?
	
	-- Chúng ta nên học cái gì? Học như thế nào? Tại sao cần phải học? Nên học ở đâu? Học khi nào? Học ai? Học với ai?
\end{question}
\noindent{\bf \textsf{Resources -- Tài nguyên.}}
\begin{enumerate}
...
\subsection{Some principles on self-learning -- Vài nguyên tắc tự học}
...
\begin{Rule}[Word]
	...
	-- Cẩn thận \& chọn lựa trong việc sử dụng ngôn từ.
\end{Rule}
...
\subsection{On question-driven learning -- Bàn về việc học được thúc đẩy bởi các câu hỏi}
...
-- Một câu hỏi hay có nhiều giá trị hơn một loạt các câu trả lời ngu ngốc, vô nghĩa hoặc trung bình.
...
\begin{quote}
	...
	-- Ái chà chà, đúng rồi, ta sẽ trở thành 1 thằng Ngu, thằng Khờ! Khi đó mọi người sẽ phải khiếp sợ ta! Muahahaha!
\end{quote}
...
\subsection{On eidetic memory -- Bàn về trí nhớ điện tử}

\begin{question}[Eidetic memory -- Trí nhớ điện tử]
	Is eidetic memory a gift or a curse?
	
	-- Trí nhớ điện tử là 1 món quà hay là 1 sự nguyền rủa?
\end{question}
...
-- Trí nhớ điện tử là 1 trong các điều kiện cần thiết nhất cho sự cô độc không thể tránh khỏi của các kẻ sở hữu trí nhớ điện tử. Tuy nhiên, bất cứ điều kiện cần nào cũng phụ thuộc vào việc những kẻ đó sử dụng trí nhớ điện tử của họ như thế nào.
...
\subsection{Importance of understanding \& then applying psychology in life -- Tầm quan trọng của việc hiểu \& sau đó ứng dụng Tâm Lý Học vào đời sống}
\textbf{\textsf{Resources -- Tài nguyên.}}
\begin{itemize}
...
-- Nhiều người hiểu lầm sức mạnh là sự tức giận \&{\tt/}hoặc sự hung hăng thống trị. Nhưng sức mạnh thực sự thực sự là {\it sự bình tĩnh dưới cơn bão}: {\it Lắng nghe \& quan sát để hiểu thay vì theo dõi \& phán đoán}.
...
-- {\it Bạn đã bao giờ để ý rằng có cụm từ ``logic'' ở cuối thuật ngữ ``tâm lý'', cụm từ này dường như coi những điều vô nghĩa{\tt/}lập luận phi logic{\tt/}sự kiện{\tt/}tình huống từ những điều nhảm nhí của con người sau khi phần logic của nó có vẻ vô dụng không?} Nếu bất cứ điều gì không logic{\tt/}không có ý nghĩa ngay từ đầu, thay vào đó bạn nên xem xét nó theo quan điểm tâm lý, khi đó có lẽ nó sẽ còn ``logic'' hơn nữa.
...
\begin{quote}
	...
	-- Tôi lại thua nữa rồi. Tôi nên làm gì giờ?
...
\subsection{On theoretical vs. practical -- Bàn về lý thuyết vs. thực dụng}
...
\begin{goal}[Scientific career -- Sự nghiệp khoa học]
	...
	-- Xây dựng lại 1 sự nghiệp làm nghiên cứu Khoa Học mới hoàn toàn cho bản thân bạn.
\end{goal}

\subsection{Some self-questions -- Vài câu hỏi tự vấn}

\begin{itemize}
	\item {\it Is the general structure of my draft{\tt/}craft good enough yet?}
	
	-- Liệu cấu trúc tổng thể của bản thảo{\tt/}công trình của tôi có đủ tốt chưa?
\end{itemize}

\subsection{Some rules \& principles}
...
\begin{Rule}[Local vs. Global -- Cục bộ{\tt/}địa phương vs. Toàn cục{\tt/}tổng thể]
	...
	-- Nếu bạn không làm được điều gì đó cục bộ, hãy làm nó toàn cục. Nếu bạn không làm được điều gì đó toàn cục, hãy làm nó cục bộ. Thay đổi tỷ lệ (microscale $\leftrightarrow$ macroscale) \& phối cảnh, góc nhìn nội quan thường xuyên, đặc biệt là khi bị kẹt.
\end{Rule}
...
\begin{quote}
	{\sf{\sc Quí} [58; high school mathematics teacher; hiền, thông minh, hiện đại, tỉ mỉ]}: 24.7.2020: Có phải em đang ở bên Đức không?
	
	{\sf Hồng [24; 1st-year Mathematics PhD student]}: 25.7.2020: Dạ, vâng thầy. Em qua Berlin hồi giữa tháng 3 để bắt đầu làm nghiên cứu sinh. Thầy khỏe hơn nhiều chưa thầy?
	...
	{\sf{\sc Quí} [58; high school mathematics teacher]}: Mỗi người có một sở trường riêng. Em chọn ngành nầy thầy cũng thích lắm. Thời đại ngày nay khoa học đã tiến quá xa, nếu không vận dụng được máy tính chỉ làm toán trên tờ giấy thì quá phí. Ngay cả dạy toán phổ thông nhiều gv không ứng dụng được các phần mềm toán cũng là hạn chế lớn. Thầy tin với năng lực của em còn tiến xa hơn nữa. Minh Thảo thương thầy lắm viết mail về cho thầy hoài.
	...
	28.7.2020: Dạ, thầy thấy khỏe hơn nhiều chưa thầy?
\end{quote}
...
Thật ra tôi biết chuyện thầy {\sc Quí} sắp mất khá lâu trước đó gần nửa năm. Tôi nói thế không phải tỏ ra mình hay ho hoặc tài năng trong việc người khác chết. Như thế thì khốn nạn quá. Nếu bạn hỏi tôi vì sao tôi biết thầy sắp mất. Đó là các dấu hiệu của 1 người tự thân họ cảm giác họ sắp ra đi \& cố để lại tất cả những gì mình đã đạt được trong cuộc đời của người đó cho lứa sau này, cho thế hệ sau này.

Tôi lên nhóm Toán Cấp 3 Bến Tre trên Facebook, thầy {\sc Quí} là quản trị viên của nhóm. Tôi thấy thầy đăng rất thường, rất nhiều, gồm tất cả những bản thảo cũ được viết tay của thầy với cái nét chữ nắn nót từng li từng tí không thể nhầm lẫn với ai được. Vài đứa bạn trong lớp chuyên Toán của tôi ảnh hưởng bởi nét chữ \& phong cách ghi chép, viết bài của thầy.

Nếu như bạn làm 1 nghề nào đó \& đạt tới 1 mức độ mà người ta hay gọi là ``lành nghề'', khi họ sắp chết, hay đúng hơn là tự thân cảm nhận mình sắp chết, họ sẽ cố truyền cái sự lành nghề đó của họ cho mọi người. Không 1 chút ích kỷ, không 1 chút toan tính, không 1 chút vụ lợi cho bản thân. Họ thấu hiểu được 1 trong những mục đích tối thượng của hành trình 1 đời người: để lại điều gì đó tốt đẹp cho thế hệ sau để nhân loại tiến lên, hoặc ít nhất cũng không thoái lùi kiểu ``tiến hóa ngược'' (backward evolution). Thầy {\sc Quí} là kiểu người như thế.

Còn nếu bạn hỏi tại sao tôi lại có thể cảm nhận tinh tế cảm xúc, cảm giác của 1 người cận tử thì bởi vì cha tôi đã làm y như thế lúc sắp chết:
...
\begin{quote}
	...
	{\sf Nhân [23; Master 2 pure mathematics students]} nghĩ trong đầu: {\it Why does this stupid bitch talk so much? Can she just shut the fuck up \& let me alone?} -- Tại sao con đĩ ngu xuẩn, nứng \st{lồn} âm vật mong manh này nói lắm thế? Bộ ả ta không thể câm mồm lại \& để tui yên được à?
\end{quote}
...
\subsubsection{On joy vs. pain \& how to balance them -- Bàn về lạc thú vs. nỗi đau \& cách cân bằng chúng}
{\bf \textsf{Resources -- Tài nguyên.}}
\begin{enumerate}
...
\begin{quote}
	...
	\begin{goal}
		...
		-- Vượt qua lằn ranh giới của thù hận \& tha thứ để có thể trưởng thành về mặt cảm xúc, tâm lý, \& tinh thần.
	\end{goal}
...
\subsection{Keywords \& phrases -- Các từ khóa \& các cụm từ}
...
\subsection{Some future plans -- Vài dự định tương lai}
...
-- Chúng tôi xin lỗi vì chúng tôi nên bắt đầu dự án này sớm hơn. Nhưng cuộc đấu tranh để trưởng thành về mặt tinh thần đủ lớn \& tàn khốc đến mức ngăn cản chúng tôi làm được điều đó.
...
-- Chắc chắn một ngày nào đó tôi sẽ chết, sớm hay muộn, nhưng tôi muốn linh hồn tôi trở nên bất tử.
