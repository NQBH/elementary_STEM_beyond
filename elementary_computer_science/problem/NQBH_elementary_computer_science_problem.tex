...
\subsection{Algebra -- Đại số}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD1, p. 14]
	(a) Cho $A = x^2 + y^2,B = x + y + A,C = xy + A - B^2$ với $x,y\in\mathbb{R}$. Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tính $A,B,C$. (b) Mở rộng bài toán cho $A_i(x,y)\in\mathbb{R}[x,y]$ với $i = 1,2,\ldots,n$, $n\in\mathbb{N}^\star$, là các đa thức 2 biến $x,y$. (c) Mở rộng bài toán cho $A_i({\bf x})\in\mathbb{R}[{\bf x}]$ với $i = 1,2,\ldots,n$, là các đa thức  hệ số thực với $n\in\mathbb{N}^\star$ biến số ${\bf x} = (x_1,x_2,\ldots,x_n)$.
\end{baitoan}
See also:
\begin{itemize}
...
\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD2, p. 15]
	(a) Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tìm {\rm GTLN} $\max\{a,b,c\}$ \& {\rm GTNN} $\min\{a,b,c\}$ với $a,b,c\in\mathbb{R}$. (b) Mở rộng bài toán cho $n\in\mathbb{N}^\star$ số $a_1,a_2,\ldots,a_n$.
\end{baitoan}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD3, p. 16]
	(a) Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tính giá trị của hàm số $f:\mathbb{R}^2\to\mathbb{R}$ xác định bởi công thức:
	\begin{equation*}
...
\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD4, p. 17]
	(a) Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tìm nghiệm của phương trình bậc $\le2$ có dạng $ax^2 + bx + c = 0$ với $a,b,c\in\mathbb{R}$. (b) Mở rộng bài toán cho phương trình bậc 3, bậc 4. (c) Mở rộng bài toán cho các phương trình đa thức có thể giải được nhờ quy về phương trình bậc 2, 3.
\end{baitoan}
See also:
\begin{itemize}
...
\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD5, p. 18]
	Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tính tổng $S = \sum_{i = 1}^{100} x_i$ với $x_i\in\mathbb{R}$, $\forall i = 1,2,\ldots,100$. (b) Mở rộng bài toán cho các tổng tương tự.
\end{baitoan}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD6, p. 19]
	Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tính giá trị của đa thức $P(x) = \sum_{i=0}^n a_ix^i = a_nx^n + a_{n-1}x^{n-1} + \cdots + a_1x + a_0\in\mathbb{R}[x]$ với hệ số thực $a_i\in\mathbb{R}$, $\forall i = 0,1,\ldots,n$ với $n\in\mathbb{N}^\star$ bằng cách sử dụng thuật toán Horner.
\end{baitoan}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD7, p. 21]
	Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tính giá trị hàm số mũ $e^x$ với $x = a$ với sai số $\varepsilon > 0$ cho trước bằng cách sử dụng khai triển Taylor của hàm $e^x$.
\end{baitoan}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD8, p. 22]
	Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để tìm nghiệm của phương trình $f(x) = x^5 - 6x^4 - 15x^3 - 20x^2 + 14x - 4 = 0$ với giá trị đầu (initial value) $x_0 = 0.8$.
\end{baitoan}

\begin{baitoan}[\cite{Doanh_Tuan_Pascal}, VD9, p. 23]
	Viết thuật toán, vẽ lưu đồ, \& viết chương trình {\sf Pascal, Python, C{\tt/}C++} để giải hệ phương trình tuyến tính cấp $n$ có dạng
	\begin{equation}
		\left\{\begin{split}
