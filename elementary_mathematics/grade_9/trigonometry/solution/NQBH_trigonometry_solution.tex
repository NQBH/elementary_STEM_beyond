...
\begin{center}
	\begin{tikzpicture}
		...
		;
		\draw
		(A)--(B)--(C)--cycle (A)--(H) (A)--(M) (H)--(I) (B)--(K)
		pic[draw, angle radius=1.5mm]{right angle=A--H--C}
		pic[draw, angle radius=1.5mm]{right angle=A--I--H}
		pic[draw, angle radius=1.5mm]{right angle=B--K--C}
		;
		\foreach \x/\g in {A/90,B/-135,C/-45,H/-90,I/45,K/45} \draw[fill=white] (\x) circle (.05) + (\g:.3) node{$\x$};
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		...
		\draw
		(A)--(B)--(C)--cycle (A)--(H) (B)--(H)
		pic[draw, angle radius=1.5mm]{right angle=B--H--C}		;
		\foreach \x/\g in {A/-90,B/90,C/-45,H/-135} \draw[fill=white] (\x) circle (.05) + (\g:.3) node{$\x$};
	\end{tikzpicture}
\end{center}
