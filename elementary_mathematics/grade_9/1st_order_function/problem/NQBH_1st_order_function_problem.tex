...
\begin{baitoan}[\cite{Binh_Toan_9_tap_1}, 53., p. 25]
	Chứng minh đồ thị của hàm số chỉ là 1 điểm: (a) $y = \sqrt{x - 3} + \sqrt{3 - x}$. (b) $y = \sqrt{x - a} + \sqrt{a - x}$, $\forall a\in\mathbb{R}$. (c) $y = \sqrt{x^n - a} + \sqrt{a - x^n}$, $\forall a\in\mathbb{R}$, $n\in\mathbb{N}$, $n$ lẻ.
\end{baitoan}
