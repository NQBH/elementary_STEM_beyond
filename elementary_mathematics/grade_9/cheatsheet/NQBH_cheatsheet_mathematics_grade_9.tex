...
\subsection{Square root -- Căn bậc 2}
\fbox{1} ĐKXĐ: Đa thức (1 biến hay nhiều biến) hệ số thực $A(x_1,x_2,\ldots,x_n)\in\mathbb{R}[x_1,x_2,\ldots,x_n]$ luôn xác định. Phân thức $\dfrac{A}{B}$ xác định $\Leftrightarrow B\ne0$. Căn thức $\sqrt{A}$ xác định $\Leftrightarrow A\ge0$. $\dfrac{A}{\sqrt{B}}$ xác định $\Leftrightarrow B > 0$. \fbox{2} $\sqrt{A^2} = |A|$. \fbox{3} $\sqrt{AB} = \sqrt{A}\sqrt{B}$, $\sqrt{A_1A_2\cdots A_n} = \sqrt{A_1}\sqrt{A_2}\cdots\sqrt{A_n}$, $\sqrt{A^2B} = |A|\sqrt{B}$. \fbox{4} $\sqrt{\dfrac{A}{B}} = \dfrac{\sqrt{|A|}}{\sqrt{|B|}} = \dfrac{\sqrt{AB}}{|B|}$. \fbox{} $(a + b\sqrt{c})^2 = a^2 + b^2c + 2ab\sqrt{c}$, $\forall a,b,c\in\mathbb{R},c\ge0$. \fbox{5} Trục căn thức ở mẫu:

\subsection{Cube root -- Căn bậc 3}

\subsection{$n$th root -- Căn bậc $n$}
...
\subsection{Phương trình quy về phương trình bậc 2}
...
\subsection{Góc ở tâm. Số đo cung. Liên hệ giữa cung \& dây}
