...
\begin{baitoan}
	Trên parabol $(P):y = ax^2 + bx + c$, $a,b,c\in\mathbb{R},a\ne0$. Tìm 2 diểm đối xứng nhau qua trục tung \& khoảng cách giữa chúng bằng $m\in[0,\infty)$.
\end{baitoan}
...
\fbox{1} Định lý Vi\`ete cho phép tính tổng \& tích 2 nghiệm của phương trình bậc 2 mà không cần xác định các nghiệm của nó:
...
\end{dinhly}
\noindent\fbox{2} {\sf Giải bất phương trình bậc 2 1 ẩn}: Giả sử tam thức bậc 2 $f(x) = ax^2 + bx + c$, $a,b,c\in\mathbb{R},a\ne0$ có 2 nghiệm $x_1,x_2$. Từ định lý dấu của tam thức bậc 2, $ax^2 + bx + c < 0\Leftrightarrow\min\{x_1,x_2\} < x < \max\{x_1,x_2\}$, $ax^2 + bx + c > 0\Leftrightarrow x < \min\{x_1,x_2\}\lor x > \max\{x_1,x_2\}$. \fbox{3} {\sf Điều kiện để bất phương trình bậc 2 có nghiệm là mọi số thực}: Cho tam thức bậc 2 $f(x) = ax^2 + bx + c$, $a,b,c\in\mathbb{R},a\ne0$. (a) $f(x) > 0,\forall x\in \mathbb{R}\Leftrightarrow a > 0\land\Delta < 0$. (b) $f(x)\ge0,\forall x\in \mathbb{R}\Leftrightarrow a > 0\land\Delta\le0$. (c) $f(x) < 0,\forall x\in \mathbb{R}\Leftrightarrow a < 0\land\Delta < 0$. (d) $f(x)\le0,\forall x\in \mathbb{R}\Leftrightarrow a < 0\land\Delta\le0$. (e) $f(x)\ge0,\forall x\in \mathbb{R}$ \& xảy ra được $f(x) = 0\Leftrightarrow a > 0\land\Delta = 0$.
