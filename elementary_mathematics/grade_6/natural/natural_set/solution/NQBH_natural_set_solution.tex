...
\begin{proof}[Giải]
	...
	\end{align*}
	Vậy cần dùng \fbox{$n(a + 1) - (10^n - 1):9$} chữ số để đánh số các trang của cuốn sách đó.
\end{proof}
...
\begin{luuy}
	Công thức tính số chữ số cần dùng để ghi các số tự nhiên liên tiếp: Gọi số các số có $1$ chữ số là $a_1$, số các số có $2$ chữ số là $a_2$, $\ldots$,  số các số có $n$ chữ số là $a_n$ (hay viết gọn: gọi số các số có $i$ chữ số là $a_i$, $\forall i = 1,2,\ldots,n$) thì số chữ số cần dùng là: \fbox{$S = \sum_{i=1}^n ia_i = a_1 + 2a_2 + \cdots + na_n$}.
\end{luuy}
