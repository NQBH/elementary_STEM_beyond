...
\fbox{1} \textit{Phương trình ẩn $x$} có dạng $A(x) = B(x)$, trong đó $A(x),B(x)$ là 2 biểu thức của cùng 1 biến $x$, trong đó $D_A,D_B$ lần lượt là TXĐ của 2 biểu thức $A,B$. Giá trị $x = x_0\in D_A\cap D_B\subset\mathbb{R}$ làm cho 2 vế của phương trình nhận cùng 1 giá trị gọi là 1 \textit{nghiệm} (\textit{root}) của phương trình. 1 phương trình có thể có 0 (i.e., vô nghiệm) $1,2,3,\ldots$ nghiệm hoặc vô số nghiệm. Tập hợp tất cả các nghiệm của 1 phương trình gọi là \textit{tập nghiệm} của phương trình đó, ký hiệu là $S$, i.e., với phương trình $f(x) = 0$, $S_f\coloneqq\{x\in D_f\subset\mathbb{R}|f(x) = 0\}$. \fbox{2} \textit{2 phương trình tương đương} là 2 phương trình có cùng 1 tập nghiệm: $(f(x) = 0\Leftrightarrow g(x) = 0)\Leftrightarrow S_f = S_g$ với $S_f\coloneqq\{x\in D_f\subset\mathbb{R}|f(x) = 0\},S_g\coloneqq\{x\in D_g\subset\mathbb{R}|g(x) = 0\}$ lần lượt là 2 tập nghiệm của 2 phương trình $f(x) = 0,g(x) = 0$, trong đó $D_f,D_g$ lần lượt là TXĐ của 2 hàm $f,g$. \textit{Tính chất bắc cầu của quan hệ tương đương của các phương trình}: 2 phương trình cùng tương đương với 1 phương trình thứ 3 thì tương đương với nhau, i.e., $[(f(x) = 0\Leftrightarrow g(x) = 0)\land(g(x) = 0\Leftrightarrow h(x) = 0)]\Rightarrow(f(x) = 0\Leftrightarrow h(x) = 0)$, hay tương đương với $[(S_f = S_g)\land(S_g = S_h)]\Rightarrow S_f = S_h$.
...
\begin{proof}[Giải]
	(a) Khi ôtô chạy $x$ km thì tiêu thụ hết $y = \dfrac{8}{100}x = \dfrac{2}{25}x = 0.08x$ L xăng. (b) $y$ là 1 hàm số bậc nhất của $x$. (c) $y = \dfrac{2}{25}x\Leftrightarrow x = \dfrac{25}{2}y = 12.5y\Rightarrow x$ là 1 hàm số của $y$. (d) $x = 200\Rightarrow y = \dfrac{2}{25}\cdot200 = 16$ L. Số lượng xăng còn lại trong bình: $40 - 16 = 24$ L.
\end{proof}
...
\begin{center}
	\begin{tikzpicture}[>=stealth]
		...
		(3,0) coordinate (M)
		(0,1) coordinate (N);
		\draw (A)--(B)--(C) (B)--(M)--(N)--cycle (A)node[left]{$5$} (C) node[below left]{$4$} (M) node[below]{$x$} (N) node[below left]{$2$};
		\foreach \x/\g in {A/45,B/45,C/-45,M/45,N/135,O/-135} \draw[fill=white] (\x) circle (.05) + (\g:.3) node{$\x$};
	\end{tikzpicture}
\end{center}

\begin{proof}[1st giải]
	(a) Tứ giác $OABC$ là hình chữ nhật có $OA = 5,OC = 4$, $S_{OABC} = OA\cdot OC = 5\cdot4 = 20$. (b) $P = S_{OABM} - S_{ABN} - S_{OMN} = \dfrac{(4 + x)\cdot5}{2} - \dfrac{3\cdot4}{2} - \dfrac{2x}{2} = \dfrac{3x}{2} + 4\Rightarrow P$ là hàm số bậc nhất của $x$. (c) TXĐ $D_P = \{x\in\mathbb{R}|x > 4\}$. Vì $P$ là hàm số bậc nhất của $x$ với hệ số góc $\dfrac{3}{2} > 0$ nên $P$ đồng biến trên $D_P$. (D) $S = P\Leftrightarrow\dfrac{3x}{2} + 4 = 20\Leftrightarrow x = \dfrac{32}{3} > 4$ (nhận vì $x\in D_P$).
\end{proof}
Để xét tính đơn điệu (i.e., đồng biến hoặc nghịch biến) của hàm số $P$, ta có thể sử dụng định nghĩa:

\begin{proof}[2nd giải]
	(c) TXĐ $D_P = \{x\in\mathbb{R}|x > 4\}$. Lấy $x_2 > x_1 > 4$: $P_2 - P_1 = \dfrac{3x_1}{2} + 4 - \left(\dfrac{3x_2}{2} + 4\right) = \dfrac{3}{2}(x_2 - x_1) > 0\Rightarrow P_2 > P_1$. Vậy hàm số $P$ đồng biến trên $D_P$.
\end{proof}
...
\begin{center}
	\begin{tikzpicture}[scale=.6]
		...
		(2.4,0) coordinate (Q)
		(1.2,0) coordinate (Y)		;
		\draw (A)--(B)--(C)--(D)--cycle (A)--(C) (B)--(D) (M)--(N)--(Q)--(P)--cycle (X) node[above]{$x$} (Y) node[below]{$36 - x$};
		\foreach \x/\g in {A/135,B/45,C/-45,D/-135,M/90,N/180,P/0,Q/-90} \draw[fill=white] (\x) circle (.1) + (\g:.5) node{$\x$};
	\end{tikzpicture}
\end{center}

\begin{proof}[Giải]
	(a) Áp dụng định lý Pythagore cho $\Delta ABC$ vuông tại A: $AC = \sqrt{AB^2 + BC^2} = \sqrt{36^2 + 15^2} = 39$. Vì $ABCD$ là hình chữ nhật, $BD = AC = 39$. Áp dụng định lý Thales cho $\Delta ABD$ với $MN\parallel BD$: $\dfrac{AM}{AB} = \dfrac{MN}{BD}\Leftrightarrow MN = \dfrac{AM\cdot BD}{AB} = \dfrac{x\cdot39}{36} = \dfrac{13}{12}x$ nên $MN$ là 1 hàm số bậc nhất của $x$. Tương tự, áp dụng định lý Thales cho $\Delta ABC$ với $MP\parallel AC$: $\dfrac{BM}{AB} = \dfrac{MP}{AC}\Leftrightarrow MP = \dfrac{BM\cdot AC}{AB} = \dfrac{(36 - x)\cdot39}{36} = -\dfrac{13}{12}x + 39$ nên $MP$ cũng là 1 hàm số bậc nhất của $x$. (b) Áp dụng định lý Thales cho $\Delta ABC$ với $MP\parallel AC$: $\dfrac{BP}{BC} = \dfrac{BM}{AB} = \dfrac{36 - x}{36} = \dfrac{DQ}{CD}$, áp dụng định lý Thales đảo cho $\Delta BCD$ suy ra $PQ\parallel BD$, mà $MN\parallel BD$ (giả thiết), suy ra $MN\parallel PQ$. Tương tự, áp dụng định lý Thales cho $\Delta ABD$ với $MN\parallel BD$: $\dfrac{DN}{AD} = \dfrac{BM}{AB} = \dfrac{36 - x}{36} = \dfrac{DQ}{CD}$, áp dụng định lý Thales đảo cho $\Delta ACD$ suy ra $QN\parallel AC$, mà $MP\parallel AC$ (giả thiết), suy ra $NQ\parallel MP$. Tứ giác $MNQP$ có 2 cặp cạnh đối song song $MN\parallel PQ$, $QN\parallel AC$, nên là hình bình hành. Suy ra $C_{MNPQ} = 2(MN + MP) = 2\left(\dfrac{13}{12}x -\dfrac{13}{12}x + 39\right) = 2\cdot39 = 78$ nên $C_{MNPQ}$ là hàm hằng (i.e., không phụ thuộc vào biến $x$).
\end{proof}
...
\begin{baitoan}[\cite{Binh_boi_duong_Toan_9_tap_1}, VD5, p. 52]
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth]
			...
			(1,-3/2) coordinate (C)
			(2,0) coordinate (D);
			\draw (A)--(B)--(C)--(D) (0,3/2) node[right]{$3$} (0,-3/2) node[below right]{$-3$} (1,0) node[above]{$2$} (2,0) node[above]{$4$} (-1/2,0) node[above]{$-1$} (-3/2,0) node[below]{$-3$};
			\foreach \x/\g in {A/90,B/-135,C/-45,D/-45,O/-135} \draw[fill=white] (\x) circle (.05) + (\g:.3) node{$\x$};
		\end{tikzpicture}
	\end{center}
...
\begin{proof}[Giải]
	(a) TXĐ của hàm số $y$: $D_y = \{x\in\mathbb{R}|-3\le x\le4\} = [-3,4]$. (b) $f(-3) = 3,f(-1) = -3,f(1) = -3,f(2) = -3,f(3) = -1.5,f(4) = 0$. (c) Hàm số $y$ nghịch biến trên đoạn $[-3,-1]$, hằng trên đoạn $[-1,2]$, đồng biến trên đoạn $[2,4]$. (d) Khẳng định hàm số $y$ nghịch biến trong đoạn $[-3,2]$ là sai, e.g., cho $x_1 = -1,x_2 = 2$, $x_1 < x_2$ nhưng $f(-1) = f(2) = -3$.
\end{proof}

\begin{luuy}
	Khi xét tính đơn điệu, i.e., đồng biến hoặc nghịch biến của 1 hàm số bất kỳ, ta xét từ trái sang phải, khoảng nào mà: Đồ thị hàm số đi lên thì hàm số đồng biến trong khoảng đó. Đồ thị hàm số đi xuống thì hàm số nghịch biến trong khoảng đó. 
\end{luuy}

\begin{luuy}
	Khi xét tính đồng biến, nghịch biến của 1 hàm số $y = f(x)$ ta phải chọn 2 giá trị tùy ý $x_1,x_2\in D_y$, $x_1 < x_2$.
\end{luuy}
...
\begin{center}
	\begin{tikzpicture}
		...
		($(A)!.5!(M)$) coordinate (X)
		($(B)!.5!(M)$) coordinate (Y);
		\draw (A)--(B)--(C)--cycle (M)--(N)--(P)--(Q)--cycle (X) node[above left]{$x$} (Y) node[left]{$a - x$} pic[draw, angle radius = 1.5mm]{right angle = M--Q--P} pic[draw, angle radius = 1.5mm]{right angle = Q--M--N} pic[draw, angle radius = 1.5mm]{right angle = M--N--P} pic[draw, angle radius = 1.5mm]{right angle = N--P--Q};
		\foreach \x/\g in {A/90,B/-135,C/-45,M/135,N/45,P/-90,Q/-90} \draw[fill=white] (\x) circle (.05) + (\g:.3) node{$\x$};
	\end{tikzpicture}
\end{center}
